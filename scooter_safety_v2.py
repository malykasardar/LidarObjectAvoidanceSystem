    ...
    def process_obstacle(self, dist, left, right):
        ...
        if dist <= STOP_DIST:
            ...
            if rospy.get_time() - self.stuck_start_time > 2.5:
                ...
                open("/tmp/lidar_stop","w").write("1")
                open("/tmp/lidar_steer","w").write("0.0")
        ...
        # AVOIDANCE STEERING
            ...
            # Only change direction if one side is clearly dominant (>3 hit difference)
            if abs(left - right) > 3:
            ...
            open("/tmp/lidar_stop","w").write("0")
            open("/tmp/lidar_steer","w").write(str(round(move_cmd.angular.z * 1.5, 3)))
